...
class XYSystem():
    def __init__(self,temperature = 3,width=10):
        ...
        self.spin_config = np.random.random(self.num_spins)*2*pi #list of spin configurations
        self.temperature = temperature
        self.energy = np.sum(self.get_energy())/self.num_spins
        self.M = []
        self.Cv = []
        self.mag = sum(self.spin_config)/self.num_spins
        
    def total_mag(self):
            total_mvec = []
            for spin in self.spin_config:
                total_mvec = total_mvec.append(spin)
            return sum(total_mvec)/self.num_spins
    ...
    #  input: S/spin configuration in list of float (0,2pi]
    ...
    def show(self,colored=False):
        ...
        print('Magnetization in Radians= %.2f'%self.mag)
    
    def show_mag(self, Ti, Tf, steps, resolution):
        data_points = np.linspace(Ti, Tf, steps)
        
        mag_list = []
        temp_list = []
        for temp in data_points:
            xy = XYSystem(temperature = temp, width = resolution)
            mag_list = mag_list + [xy.mag] 
            temp_list = temp_list + [xy.temperature]
            xy.show()
        print(mag_list)
        print(temp_list)
